...
include_once ('conexao.php');
    ...
    $result_comunicado = "SELECT publicacao.titulo, publicacao.data_publicacao, publicacao.autor LIMIT $lista_inicio, $limite_dados;";
    $resultado_comunicado = mysqli_query($poti_con, $result_user);
    if(($resultado_comunicado) AND ($resultado_comunicado->num_rows != 0 )){
        while($row_comunicado = mysqli_fetch_assoc($resultado_comunicado)){
            echo "<tr>
                    <th><input type='checkbox' id='vehicle3' name='vehicle3' value=".$row_user['cod_publicacao']."></th>
